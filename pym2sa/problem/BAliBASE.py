...
class BAliBASE(MSA):
    ...
    def __init__(self, balibase_instance: str, balibase_path: str, score_list: List[Score]) -> None:
        ...
        super(BAliBASE, self).__init__(score_list=score_list, sequences_without_gaps=[], sequences_names=[])
    ...
    def __import_instance(self):
        sequences = self.__read_sequences_without_gaps_as_list_of_pairs()

        self.sequences_names = list(pair[0] for pair in sequences)
        self.sequences_without_gaps = list(pair[1] for pair in sequences)
    ...
    def __read_sequences_without_gaps_as_list_of_pairs(self) -> List[str]:
        bb3_release_path = self.__compute_path('bb_release')
        logger.info('Reading sequences without gaps from {}'.format(bb3_release_path))

        if os.path.isdir(bb3_release_path):
            sequences = read_fasta_file_as_list_of_pairs(self.balibase_instance + '.tfa', bb3_release_path)
        else:
            raise Exception('Instance not found at {}'.format(bb3_release_path))

        logger.info('...OK ({} instances found)'.format(len(sequences)))
