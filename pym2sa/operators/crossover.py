...
class GapSequenceSolutionSinglePoint(Crossover[MSASolution, MSASolution]):
    ...
    def do_crossover(self, parents: List[MSASolution]) -> List[MSASolution]:
        if random.random() <= self.probability:
            ...
            column_positions_in_second_parent = \
                self.find_original_positions_in_aligned_sequence(parents[1], column_positions_in_first_parent)
    ...
    def find_original_positions_in_aligned_sequence(self,  solution: MSASolution, column_positions_in_first_parent: list):
        positions = []
